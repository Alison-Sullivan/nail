...
start %root%
:nalselect
echo  NAL Local or Remote?
set /p nalsel= L/R:
if %nalsel%==R (set nalsel=true)
if %nalsel%==r (set nalsel=true)
if %nalsel%==true (goto nal_remote) else (goto nal_local)
...
goto lstart
:changelog
if exist %root%\log_change.txt echo  The change in network setup has been logged in C:\nal_data\log_change.txt
...
set /p nalr_con= Y/N:
if %nalr_con%==Y (set nalr_con=true)
if %nalr_con%==y (set nalr_con=true)
cls
if %nalr_con%==true (echo>nul OK) else (goto nalselect)
echo.
